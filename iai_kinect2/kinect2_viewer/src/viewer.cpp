...
private:
    ...
    // Callback is only called if a message has been received on all specified channels with the same or approximate exact timestamp.
...
public:
    Receiver(const std::string &topicColor, const std::string &topicDepth, const bool useExact,
            ...
            : topicColor(topicColor), topicDepth(topicDepth), useExact(useExact), useCompressed(useCompressed),
              updateImage(false), updateCloud(false), save(false), running(false), frame(0), queueSize(5), nh("~"),
              spinner(0), it(nh), mode(CLOUD) {
...
private:
    void start(const Mode mode) {
        ...
        subCameraInfoColor = new message_filters::Subscriber<sensor_msgs::CameraInfo>(nh, topicCameraInfoColor,
                                                                                      queueSize);
        subCameraInfoDepth = new message_filters::Subscriber<sensor_msgs::CameraInfo>(nh, topicCameraInfoDepth,
                                                                                      queueSize);

        if (useExact) {
            syncExact = new message_filters::Synchronizer<ExactSyncPolicy>(ExactSyncPolicy(queueSize), *subImageColor,
                                                                           *subImageDepth, *subCameraInfoColor,
                                                                           *subCameraInfoDepth);
    ...
    void saveCloudAndImages(const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr cloud, const cv::Mat &color,
                            const cv::Mat &depth, const cv::Mat &depthColored) {
        oss.str("");
...
#pragma omp paraller for
        for (int r = 0; r < depth.rows; r++) {
            ...
            for (size_t c = 0; c < (size_t) depth.cols; ++c, ++itP, ++itD, ++itC, ++itX) {
                // using register variable indicates the variable is always saved in the CPU Register.
...
int main(int argc, char **argv) {
    ...
    OUT_INFO("topic color: "
                     FG_CYAN << topicColor << NO_COLOR);
    OUT_INFO("topic depth: "
                     FG_CYAN << topicDepth << NO_COLOR);
