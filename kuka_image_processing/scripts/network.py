...
def plot_output(rgb_img, depth_img, grasp_position_img, grasp_angle_img, ground_truth_bbs, no_grasps=1,
    ...
    gs = detect_grasps(grasp_position_img, grasp_angle_img, width_img=grasp_width_img, no_grasps=no_grasps,
                       ang_threshold=0)
...
def calculate_iou_matches(grasp_positions_out, grasp_angles_out, ground_truth_bbs, no_grasps=1, grasp_width_out=None,
                          min_iou=0.25):
    """
...
def run():
    ...
    for model_folder in model_folders:
        ...
        for epoch in epochs:
            ...
            # IOU TESTING.
            succeeded, failed = calculate_iou_matches(grasp_positions_out, grasp_angles_out, bbs_all,
                                                      no_grasps=NO_GRASPS, grasp_width_out=grasp_width_out)
            ...
            if VISUALISE_FAILURES:
                ...
                shuffle(failed)
                for i in failed:
                    plot_output(rgb_imgs[i,], depth_imgs[i,], grasp_positions_out[i,].squeeze(),
                                grasp_angles_out[i,].squeeze(), bbs_all[i,],
