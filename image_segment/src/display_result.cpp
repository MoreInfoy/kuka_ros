...
void rgb_callback(const sensor_msgs::ImageConstPtr &image)
{
    try
    {
        ...
        int col = int(position) % 300;
        int row = (int(position) - col) / 300;
...
void depth_callback(const sensor_msgs::ImageConstPtr &image)
{
    ...
    convertScaleAbs(depth, roi_image, 400.0, -500);
    adaptiveThreshold(roi_image, roi_threshold, 250.0, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY_INV, 7, 2);
